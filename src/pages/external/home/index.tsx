  ...
  return (
    <div className='flex flex-col w-full h-auto'>
      ...
      <section className='w-full container flex  flex-col items-center gap-[3rem] mb-[8.06rem]'>
        ...
        <div className='flex w-full max-w-full pl-container-base lg:pl-container-lg'>
          <EmptyContentWrapper isEmpty={!blogLoading && blogs?.items && blogs?.items?.length < 1}>
            <ContentLoader isLoading={blogLoading}>
              <Carousel className='w-full max-w-full' perView={'auto'} spacing={16}>
                {blogs?.items?.map((i, idx) => (
                  <Carousel.Child key={idx} className=''>
        ...
        <div className='px-container-base lg:px-container-lg w-full flex justify-end gap-[0.6875rem] items-center group text-primary-1 cursor-pointer'>
          <span
            onClick={() => navigate(`/${CONSTANTS.ROUTES.blogs}`)}
            className='font-[500] text-[18px] leading-[24px] tracking-[0.15px] group-hover:underline transition-all ease-in-out'
          >
            View in full
          </span>
      ...
      <section className='w-full container flex  flex-col items-center gap-[3rem] mb-[8.06rem] px-container-base lg:px-container-lg'>
        ...
        <div className='flex w-full max-w-full'>
          <EmptyContentWrapper isEmpty={!btsLoading && bts?.items && bts?.items?.length < 1}>
            <ContentLoader isLoading={btsLoading}>
              <Carousel className='w-full' perView={'auto'} spacing={16}>
                {bts?.items?.map((i, idx) => (
                  <Carousel.Child key={idx} className=''>
                    <div className='min-w-[25.69rem]'>
                      <BtsCard
